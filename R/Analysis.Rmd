...
```{r}
results %>% 
  filter(agent_count == 1000) %>% 
  group_by(runid) %>% 
    arrange(desc(step)) %>% 
    slice(1) %>% 
  ungroup() %>% 
  select(agent_count, agent_generator, network_generator, content_type, seen) ->
temp_data

temp_data %>% 
  glm(seen ~ agent_generator + network_generator + content_type, data = .) -> mdl
